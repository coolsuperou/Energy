...
public class EnergyDataController {
     ...
     */
    @GetMapping("/alerts")
    public Result<Map<String, Object>> getAlerts(HttpSession session) {
        User currentUser = (User) session.getAttribute("user");
        if (currentUser == null) {
            return Result.error("未登录");
        }

        Map<String, Object> alertData = energyDataService.getAlerts(currentUser);
        return Result.success(alertData);
    }
}
