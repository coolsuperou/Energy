    ...
    <div class="row g-4 mb-4">
      <div class="col-md-3">
        <div class="stat-card text-center">
          <div class="stat-value text-primary">{{ stats.monthlyApproved }}</div>
          <div class="stat-label">本月审批工单</div>
        </div>
      </div>
      <div class="col-md-3">
        <div class="stat-card text-center">
          <div class="stat-value text-success">{{ stats.monthlyDispatched }}</div>
          <div class="stat-label">本月派单数</div>
        </div>
      </div>
      <div class="col-md-3">
        <div class="stat-card text-center">
      ...
      <div class="col-md-3">
        <div class="stat-card text-center">
          <div class="stat-value" style="color: #8b5cf6;">{{ stats.totalProcessed.toLocaleString() }}</div>
...
async function loadUserInfo() {
  try {
    ...
    if (res && res.code === 200 && res.data) {
      userInfo.value = {
        name: res.data.name || '',
        roleName: res.data.roleName || '能源调度员',
...
async function loadStats() {
  try {
    ...
    if (res?.code === 200 && res.data) {
      const d = res.data
      const metrics = d.todayMetrics || {}
      stats.value = {
        monthlyApproved: metrics.todayApprovedCount || 0,
        monthlyDispatched: metrics.todayCompletedTaskCount || 0,
        pendingTasks: d.pendingApplicationCount || 0,
        totalProcessed: d.inProgressTaskCount || 0
      }
    }
